...
with open(os.path.join(file_data["file_paths"]["BuildingData"], file_data["file_names"]["mic_types"]), 'r') as f:
    tp = json.load(f)
modular_type = tp['case2']                          # selecting modular size configuration
...
# run_GA
reload(GA)
pop_ori = GA.generate_inital_population(DNA_digits, region_index, len(modular_type), 150)
...
tp = ut.output_layouts(modular_plan_x, casenum)
ut.output_history_bestind(best_ind_hist, casenum)
