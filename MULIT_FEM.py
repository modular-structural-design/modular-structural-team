...
def thread_sap(File_Path,ModelPath,mySapObject_name,SapModel_name,num_thread,pop2,mic_FEM_data,FEM_sematics,modular_num,FEA_info2):
    ...
    for i in range(len(pop2)):
        q.put(i)
    for i in range(num_thread):
        t = threading.Thread(target=mulitrun_GA_1, args=(File_Path[i],ModelPath[i],mySapObject_name[i],SapModel_name[i],pop2,q,mic_FEM_data,FEM_sematics,modular_num,FEA_info2))
        t.start()
        threads.append(t)
    for i in threads:
        i.join()
...
def mulitrun_GA_1(File_Path,ModelPath,mySapObject, SapModel,pop_all,q,mic_FEM_data,FEM_sematics,modular_num,FEA_info2):
    while True:
        if q.empty():
            break
        time = q.get()
